...
export default function HandDrawSVG() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      // Split text into spans once and set initial opacity
      const originalText = text.textContent || "about us";
      text.textContent = "";
      ...
      // Create the timeline for scrub animation
      ...
      // Attach the timeline to ScrollTrigger for scrub-based animation
      ScrollTrigger.create({
        trigger: containerRef.current,
        start: "top top", // Start when the top of the container hits the top of the viewport
        end: "+=3000", // Adjust this value based on desired scroll distance (e.g., 3000px of scroll to complete the animation)
        scrub: true, // Tie animation progress to scroll position
        pin: true, // Pin the container in view while scrolling through the animation
        animation: tl,
      });
    }, containerRef);
  ...
  // Assume the JSX return is here, e.g.:
  // return <div ref={containerRef}> {/* SVGs and .animated-text */} </div>;
