...
const AboutUs = () => {
  ...
  const { scrollYProgress } = useScroll({
    target: containerRef,
    offset: ["start end", "end center"], // Trigger from when container enters viewport to when it reaches center
  });

  // Smooth scroll progress with spring effect
  ...
  // Transform scroll progress for container animations
  const opacity = useTransform(springProgress, [0, 0.4], [0, 1]);
  const scale = useTransform(springProgress, [0, 0.3], [0.8, 1]);
  const y = useTransform(springProgress, [0, 0.3], [100, 0]);

  // Path length animations for drawing effect
  const pathLength1 = useTransform(springProgress, [0.3, 0.5], [0, 1]); // First SVG paths
  const pathLength2 = useTransform(springProgress, [0.4, 0.6], [0, 1]); // Second SVG paths
  const pathLength3 = useTransform(springProgress, [0.5, 0.7], [0, 1]); // Third SVG paths

  // Circle animations (opacity and scale)
  const circleOpacity = useTransform(springProgress, [0.2, 0.4], [0, 1]);
  const circleScale = useTransform(springProgress, [0.2, 0.4], [0.5, 1]);
